...
export const ChatVadInput = memo(() => {
    ...
    const vad = useMicVAD({
        ...
        // model: "v5",
        redemptionMs: 3000,      // 停顿3秒后认为说话结束（调整为3秒，更自然）
        minSpeechMs: 400,        // 最小说话时长400ms
...
export const ChatStreamInput = memo(() => {
    ...
    useEffect(() => {
        console.log('[ChatInput] Setting up live2d:touch event listener');
        
        // Live2D触摸事件监听器
        const handleLive2dTouch = async (event: Event) => {
            const customEvent = event as CustomEvent;
            const { text } = customEvent.detail;
            console.log('[ChatInput] ===== Live2D touch event received =====');
            console.log('[ChatInput] Touch text:', text);
            
            // 直接调用TTS播放语音，不通过AI
            try {
                console.log('[ChatInput] Starting TTS synthesis...');
                ...
                console.log('[ChatInput] TTS Engine:', ttsEngine);
                console.log('[ChatInput] TTS Config:', ttsConfig);
                
                const controller = new AbortController();
                const audioBase64 = await api_tts_infer(ttsEngine, ttsConfig, text, controller.signal);
                
                console.log('[ChatInput] TTS API response:', audioBase64 ? 'Got audio data' : 'No audio data');
                
                if (audioBase64) {
                    // TTS返回的是base64编码的音频数据
                    const audioData = base64ToArrayBuffer(audioBase64);
                    console.log('[ChatInput] Converted to ArrayBuffer, size:', audioData.byteLength);
                    
                    const wavBuffer = await convertMp3ArrayBufferToWavArrayBuffer(audioData);
                    console.log('[ChatInput] Converted to WAV, size:', wavBuffer.byteLength);
                    
                    const manager = Live2dManager.getInstance();
                    manager.pushAudioQueue(wavBuffer);
                    console.log('[ChatInput] Audio pushed to queue');
                    
                    // 立即播放音频
                    const result = manager.playAudio();
                    console.log('[ChatInput] PlayAudio result:', result);
                    console.log('[ChatInput] ===== Touch voice synthesized and playing =====');
                } else {
                    console.error('[ChatInput] TTS synthesis failed, no audio data returned');
                }
            } catch (error) {
                console.error('[ChatInput] Failed to synthesize touch voice:', error);
            }
        };
        
        document.addEventListener('live2d:touch', handleLive2dTouch);
        console.log('[ChatInput] Event listener registered');
